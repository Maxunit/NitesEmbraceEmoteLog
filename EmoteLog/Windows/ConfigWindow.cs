...
public class ConfigWindow : Window, IDisposable
{
    ...
    public ConfigWindow(Plugin plugin) : base(
        "Nite's Embrace Emote Log Configuration",
    ...
    public override void Draw()
    {
        ...
        if (ImGui.Checkbox("Show clear button in the Nite's Embrace Emote Log", ref showClearButton))
        {
            Configuration.ShowClearButton = showClearButton;
            Configuration.Save();
        }
        if (ImGui.Checkbox("Wrap Nite's Embrace Emote Log text", ref wrapText))
        {
        ...
        if (ImGui.Checkbox("Allow moving the Nite's Embrace Emote Log", ref moveWindow))
        {
            Configuration.MoveWindow = moveWindow;
            Configuration.Save();
        }
        if (ImGui.Checkbox("Allow resizing the Nite's Embrace Emote Log", ref resizeWindow))
        {
            Configuration.ResizeWindow = resizeWindow;
            Configuration.Save();
        }
        if (ImGui.Checkbox("Show Nite's Embrace Emote Log's frames", ref showWindowFrames))
        {
            Configuration.ShowWindowFrames = showWindowFrames;
            Configuration.Save();
        }
        if (ImGui.Checkbox("Show Nite's Embrace Emote Log in combat", ref inCombat))
        {
            Configuration.InCombat = inCombat;
            Configuration.Save();
        }
        if (ImGui.Checkbox("Show Nite's Embrace Emote Log in instance", ref inInstance))
        {
            Configuration.InInstance = inInstance;
            Configuration.Save();
        }
        if (ImGui.Checkbox("Show Nite's Embrace Emote Log in cutscene", ref inCutscenes))
        {
            Configuration.InCutscenes = inCutscenes;
            Configuration.Save();
        }
        if (ImGui.Checkbox("Hide Nite's Embrace Emote Log when empty", ref hideEmpty))
        {
        ...
        {
            ImGui.SetNextItemWidth(120f);
            if (ImGui.InputFloat("Nite's Embrace Emote Log font size", ref fontSize, 1f, 2f, "%.1fpt"))
            {
